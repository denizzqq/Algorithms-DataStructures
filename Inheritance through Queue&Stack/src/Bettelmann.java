...
public class Bettelmann {
    ...
    public void playRound() {
        ...
        Card firstPlayersCard;
        Card secondPlayersCard;

        do {
            firstPlayersCard = closedPile1.pop();
            secondPlayersCard = closedPile2.pop();

            first.add(firstPlayersCard);
            second.add(secondPlayersCard);

        } while (firstPlayersCard.compareTo(secondPlayersCard) == 0 && (!closedPile1.isEmpty() && !closedPile2.isEmpty()));

        if (firstPlayersCard.compareTo(secondPlayersCard) > 0) {
            closedPile1.addAll(first);
            closedPile1.addAll(second);
        } else if (firstPlayersCard.compareTo(secondPlayersCard) < 0) {
            closedPile2.addAll(second);
            closedPile2.addAll(first);
        }
...
*/
        ...
        int round = 0;
        while (round < 6 && game.getWinner()<0) {
            round++;
            game.playRound();
